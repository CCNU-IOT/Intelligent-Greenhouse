 ...
 * htim.Init.Prescaler(psc) = SystemCoreClock / 1000 - 1; // 分频因子，使计数器每毫秒增加1
 * htim.Init.Period(arr) = 1 - 1;                         // 计数器周期为1，产生1毫秒中断
 *****************************************************************************************/ 

void TIM6_init(uint16_t arr, uint16_t psc)                 //基本定时器TIM6定时中断初始化
{
    htim.Instance = TIM6;                                 // 基本定时器
    htim.Init.Prescaler = psc;                            // 预分频系数
    htim.Init.CounterMode = TIM_COUNTERMODE_UP;           // 递增计数模式
    htim.Init.Period = arr;                               // 自动重装载值
    HAL_TIM_Base_Init(&htim);

    HAL_TIM_Base_Start_IT(&htim);                         // 使能定时器和定时器更新中断
}
...
{
    HAL_TIM_IRQHandler(&htim);                            // 定时器公共中断服务函数
}
