...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef *htim) // 此函数会被HAL_TIM_PWM_Init()调用
{
    ...
    {
        __HAL_RCC_GPIOB_CLK_ENABLE(); // 为使用定时器2通道2（重映射至PB3），使能GPIOB时钟
        __HAL_RCC_TIM2_CLK_ENABLE(); // 使能TIM2时钟
        __HAL_RCC_AFIO_CLK_ENABLE(); // 使能AFIO时钟

        GPIO_InitTypeDef gpio_init_struct;
        gpio_init_struct.Pin = WS2812B_PWM_TIM_CH2_PIN; // 设置GPIO引脚为WS2812B_PWM_TIM_CH2_PIN（GPIO_PIN_3）
        gpio_init_struct.Mode = GPIO_MODE_AF_PP; // 复用推挽输出,作为TIM2的CH2输出
        gpio_init_struct.Pull = GPIO_NOPULL;           
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;     
        HAL_GPIO_Init(WS2812B_PWM_TIM_CH2_PORT, &gpio_init_struct);

        __HAL_AFIO_REMAP_TIM2_ENABLE(); // 启用TIM2的重映射
    }
}
